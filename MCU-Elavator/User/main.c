...
int main(void) {
	...
	char moving; // remembers the direction of motion of elevator before the elevator is stopped
	...
	while(1) {
		...
		if(butt_f1||butt_f2_down||butt_f2_up||butt_f3||butt_car_stop||butt_car_reset) { 			//if any button is activated
			...
			if(butt_car_stop) {					//emergency stop activated. stops motion, but doors don't open
			...
			else if(butt_car_reset == 1) {						//if the car needs to be reset
      ...
      //if a higher floor is called, it's not already moving, and it's ready to go
					else if((butt_f2_down||butt_f2_up||butt_f3)&&door_closed) { 
		...
		//if a lower floor is called, it's not already moving, and it's ready to go
					else if(butt_f1 && door_closed) {
						if(usart_on) {
		...
		//if a lower floor is called, it's not already moving, and it's ready to go
					else if((butt_f1 || butt_f2_down || butt_f2_up)&&door_closed) {
						if(usart_on) {
...
char init(void) { 		//itianize all the pins and whatever else
	...
	TM_GPIO_Init(GPIOE, GPIO_Pin_7 | GPIO_Pin_9 | GPIO_Pin_11 | GPIO_Pin_13 | GPIO_Pin_8 | GPIO_Pin_10 | GPIO_Pin_15, TM_GPIO_Mode_OUT, TM_GPIO_OType_PP, TM_GPIO_PuPd_NOPULL, TM_GPIO_Speed_High);
...
char bcd_display(void){   // function to control the bcd display
	switch(loc_cur) {
		case 1 : {
